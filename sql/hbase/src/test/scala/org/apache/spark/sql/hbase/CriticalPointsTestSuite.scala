...
class CriticalPointsTestSuite extends FunSuite with BeforeAndAfterAll with Logging {
  ...
  test("Generate CP Ranges 0") {
    ...
    val column1Id = relation.output.find(_.name == "column1").get.exprId

    val lll = AttributeReference("column1", IntegerType)(column1Id, Seq("testTable"))
    val llr = Literal(1023, IntegerType)

    val ll = GreaterThan(lll, llr)
    val lrl = AttributeReference("column1", IntegerType)(column1Id, Seq("testTable"))
    val lrr = Literal(1025, IntegerType)

    val lr = LessThan(lrl, lrr)

    val l = And(ll, lr)
    val rll = AttributeReference("column1", IntegerType)(column1Id, Seq("testTable"))
    val rlr = Literal(2048, IntegerType)

    val rl = GreaterThanOrEqual(rll, rlr)
    val rrl = AttributeReference("column1", IntegerType)(column1Id, Seq("testTable"))
  ...
  test("Generate CP Ranges 1") {
    ...
    val lll = AttributeReference("column1", LongType)(column1Id, Seq("testTable"))
    val llr = Literal(1023L, LongType)

    val ll = GreaterThan(lll, llr)
    val lrl = AttributeReference("column1", LongType)(column1Id, Seq("testTable"))
    ...
    val l = Not(l_0)
    val rll = AttributeReference("column1", LongType)(column1Id, Seq("testTable"))
  ...
  test("Generate CP Ranges 2") {
    ...
    val lll = AttributeReference("column1", StringType)(column1Id, Seq("testTable"))
    val llr = Literal("aaa", StringType)

    val ll = EqualTo(lll, llr)
    val lrl = AttributeReference("column1", StringType)(column1Id, Seq("testTable"))
    val lrr = Literal("bbb", StringType)

    val lr = EqualTo(lrl, lrr)

    val l = Or(ll, lr)
    val rl = AttributeReference("column1", StringType)(column1Id, Seq("testTable"))
  ...
  test("Generate CP Ranges for Multi-Dimension 0") {
    ...
    val column1Id = relation.output.find(_.name == "column1").get.exprId
    val column2Id = relation.output.find(_.name == "column2").get.exprId
    val column3Id = relation.output.find(_.name == "column3").get.exprId

    val lll = AttributeReference("column3", ShortType)(column3Id, Seq("testTable"))
    val llr = Literal(8.toShort, ShortType)

    val ll = GreaterThan(lll, llr)
    val lrl = AttributeReference("column2", IntegerType)(column2Id, Seq("testTable"))
    val lrr = Literal(2048, IntegerType)

    val lr = EqualTo(lrl, lrr)

    val l = And(ll, lr)
    val rll = AttributeReference("column1", StringType)(column1Id, Seq("testTable"))
    val rlr = Literal("abc", StringType)

    val rl = EqualTo(rll, rlr)
    val rrl = AttributeReference("column1", StringType)(column1Id, Seq("testTable"))
  ...
  test("Generate CP Ranges for Multi-Dimension 1") {
    ...
    val lll = AttributeReference("column2", IntegerType)(column2Id, Seq("testTable"))
