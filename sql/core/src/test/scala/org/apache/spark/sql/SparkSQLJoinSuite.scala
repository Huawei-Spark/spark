...
class SparkSQLJoinSuite extends FunSuite {
  ...
  test("Basic Join on vanilla SparkSql: Simple Two Way  2 cols") {
    ...
    import org.apache.spark.sql.{JoinTable2Cols => JoinTab, _}
    val sc = new SparkContext("local[1]","BasicJoinTest")
    val ssc = new SQLContext(sc)
    import ssc._
    val rdd1 = sc.parallelize((1 to 2).map { ix => JoinTab(ix, s"valA$ix")})
    ...
    val rdd2 = sc.parallelize(ids.map { case (ix, is) => JoinTab(ix, s"valB$is")})
